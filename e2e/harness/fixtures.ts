...
export async function materializeHome(home: string): Promise<void> {
  ...
  // exercises the post-rename read-fallback (config.ts reads ~/.clops/ first,
  ...
  // state.json in the NEW ~/.clops/ dir pins the backend to Azure DevOps. This
  ...
  const stateDir = join(home, ".clops");
  ...
  // features / `clops status … feature/login`) is a regression guard for that
...
export const tmuxState = {
  ...
  // both the menu's readiness poll and the `clops list/status` CLI paths. The
