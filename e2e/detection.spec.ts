...
// shows. These are the pure core the TUI filter and `clops list <dir>` share.
test.describe("resolveContext: path → (filterRoot, hostSession)", () => {
  test("no path is the global launcher (null root, default session)", () => {
    expect(resolveContext(undefined, "/home/me")).toEqual({ filterRoot: null, hostSession: "clops" });
    expect(resolveContext("", "/home/me")).toEqual({ filterRoot: null, hostSession: "clops" });
  });

  test("a relative path resolves against cwd; host session is the basename", () => {
  ...
  test("-s overrides the derived host session name (basename collisions)", () => {
  ...
  test("basename is sanitized to a tmux-safe session name", () => {
    // tmux forbids `.`/`:` in session names — collapsed to `-`.
    expect(resolveContext("/repos/my.app", "/x").hostSession).toBe("my-app");
    // A path whose basename sanitizes to nothing falls back to the default.
    expect(resolveContext("/", "/x").hostSession).toBe("clops");
  });
});
