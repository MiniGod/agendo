 ...
 * is unscoped (bare `clops`). Path-scoped launchers derive their own host
 * session name (see context.ts), so every launcher-session helper below takes an
 * explicit session param defaulting to this — keeping the bare-`clops` path
 * byte-identical to before.
 */
export const LAUNCHER_SESSION = "clops";
 ...
 * `clops` session (bare `clops`); a path-scoped launcher passes its own name.
 *
