import { isRunning, refreshLiveTmux, type LoadedModel } from "../../app/model/index.ts";
import { moveSessionToProfile, type ClaudeProfile } from "../../sessions/profiles/index.ts";
import { retargetRestoreProfile } from "../../runtime/restore/index.ts";
import { profileChoices, discoverProfiles } from "../../sessions/profiles/index.ts";
...
/**
 * Moving a Claude session's transcript to another `~/.claude*` profile.
 *
 * Same shape as `makeCloneActions`: plain closures, no hooks. The one piece of
 * React state the flow needs — `moveInFlight` — stays a `useRef` in App and is
 * passed in as a plain `{ current }` box, so this module cannot perturb hook
 * order and App keeps every `useRef` call it had, in the order it had them.
 */
export function makeProfileActions({
  ...
  setActivity: (a: Map<string, Activity>) => void;
}) {
