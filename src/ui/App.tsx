...
export default function App({
  ...
  // Both free-session entry points need repos to pick from; without any, the
  // flow has nowhere to run, so say why instead of opening an empty picker.
  const haveRepos = () => {
    if (model && scopedRepos.length > 0) return true;
    setNotice(
      scoped
  ...
  useInput((input, key) => {
    ...
    if (mode.kind === "repo") {
      ...
      if (key.return && repos[mode.cursor]) {
        ...
        // Orchestrators land on "Main repo checkout": that's where the main branch
        // lives, and merging is their whole job (see the wtchoice hint).
        if (mode.target.kind === "free")
          return setMode({ kind: "wtchoice", target: mode.target, agent: mode.agent, repo, cursor: mode.target.orchestrator ? 1 : 0 });
        return setMode({
          kind: "branch",
          target: mode.target,
          agent: mode.agent,
