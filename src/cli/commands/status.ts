...
// poll it.
//
// `branchSync` is INJECTED rather than imported. src/repositories/gitRefs.ts reads a
...
import { formatResetTime, paneResetAt } from "../../sessions/usageLimit/index.ts";
import { SELF_CMD } from "../../launch/index.ts";
import { SessionIndex, loadActivity } from "../../sessions/index.ts";
import { findPeer } from "../../orchestration/peer.ts";
import { durationLabel, idleSeconds, isStalled, resolveStalledAfterMs, shortAge } from "../../sessions/idle.ts";
import { scopeFilter, scopeNote, type SessionScope } from "../../app/scope.ts";
import { refreshLiveTmux } from "../../app/model/index.ts";
import { resumeDialogChoice } from "../../app/config.ts";
import { linkLine, linkVocab } from "../output.ts";
import type { AgentSession, BranchSync, BranchSyncReader, WorkflowDetails, WorkflowRef, WorkflowStatus } from "../../shared/types.ts";
import { loadWorkflowDetails, workflowStatus } from "../../orchestration/workflows.ts";
import { flushWarnings } from "../warnings.ts";
import { readyCell, rowCompactionPercent, timeAgo } from "../list/cells.ts";
...
export async function runStatus(
  ...
  // process, no fetch — see src/repositories/gitRefs.ts). Silent when it can't be determined.
