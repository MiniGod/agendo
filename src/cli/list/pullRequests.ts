...
import type { PRWithSessions } from "../../shared/types.ts";
import { approvalInline, padCell } from "../../ui/format/index.ts";
...
export function prRows(model: Pick<LoadedModel, "linkedPrs" | "orphanPrs" | "liveTmux">) {
  ...
  return prs.map((pr) => ({
    ...
    // (src/providers/github/index.ts voteSummary), so a two-approval gate reads satisfied at
