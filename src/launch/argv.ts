...
import { orchestratorRoleOf, type OrchestratorRole } from "../orchestration/index.ts";
import { systemPromptForRole } from "../orchestration/global.ts";
import { SELF_CMD, withSelfCmdEnv } from "./selfCommand.ts";
import { launcherSystemPrompt } from "./prompt.ts";
...
export function resumeArgv(s: AgentSession): string[] {
  switch (s.source) {
    case "claude": {
      ...
      // session. Re-inject from our own marker file (see src/orchestration/index.ts) — at
