...
export function llmGuide(): string {
  return [
    ...
    "  from fell-over, so read the final response before deciding. --json adds per session:",
    "  idleSeconds, stalled, stalledAfterSeconds (the threshold it was judged against), and",
    "  git — the checkout's branch/upstream and whether it holds commits the remote does",
    "  not (unpushed). \"idle for hours AND unpushed work\" is the parked-session signal.",
    "",
