...
export function llmGuide(): string {
  return [
    ...
    "  Sends a follow-up prompt. A claude session queues it even mid-turn, so you don't",
    "  have to wait for one to go idle first; a session without that channel (Copilot)",
    "  still needs an idle input and refuses otherwise (--force to override). Either way",
    "  it refuses a session at its usage limit — check back with `status` or `wait`.",
    "",
