...
if (process.argv[2] === "send") {
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    // token, so recognizing it mid-prompt would eat a word of the message.
    else if (a === "--timeout" && parts.length === 0) dialogWaitMs = requireDuration("--timeout", rest[++i], "send");
...
function requireDuration(flag: string, s: string | undefined, cmd = "wait"): number {
