...
export function orchestratorSystemPrompt(selfCmd: string): string {
  return [
    ...
    "branch (`git -C <repo-root> …`). Not in your own worktree, if you have one: that sits",
    "on its own branch, so merging there lands the work nowhere useful. And never in a",
    "background session's worktree — that one belongs to the agent working in it. (If you",
    "were started without a worktree of your own, your cwd already IS the main checkout:",
    "merge right where you are.)",
    "",
