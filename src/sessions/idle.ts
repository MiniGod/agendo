...
//
// The default itself (4 hours) lives in src/app/config.ts next to the config shape.
...
import { DEFAULT_STALLED_AFTER_MINUTES, loadConfig } from "../app/config.ts";
import { sessionFinished, type Readiness } from "../runtime/tmux/index.ts";
 ...
 *    `git.unpushed`, see src/repositories/gitRefs.ts) already tells the story.
