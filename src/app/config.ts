...
import { parseJsonFileOr } from "../shared/errors.ts";
import type { ProviderName } from "../shared/types.ts";
import type { ResumeDialogChoice } from "../runtime/tmux/index.ts";
...
export interface Config {
   ...
   * "stalled" in `agendo list` / `agendo status` (see src/sessions/idle.ts). Overridable
   ...
   * (src/orchestration/peer.ts). Default true. Set false to force the tmux keystroke path
...
/**
 * Default stall threshold, in minutes. Lives here (not in src/sessions/idle.ts) so
...
export interface LauncherState {
   ...
   * OFF — toggled from the Settings page. See src/sessions/usageLimit/index.ts.
   */
