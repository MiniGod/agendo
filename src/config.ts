...
// in ~/.clops/config.json (see the Config interface below for the shape).
//
// On-disk paths: reads try the new `~/.clops/` first and fall back to the
...
// New data dir (`~/.clops/`) — all writes go here. `OLD_STATE_DIR` is read-only,
// used as a fallback for reads so existing installs keep working pre-migration.
export const STATE_DIR = join(homedir(), ".clops");
 ...
 * Used so reads transparently migrate from the historical `~/.claude-launcher/`
 * path while writes always go to the new `~/.clops/` location.
 */
...
export function loadConfig(): Config {
  const path = firstExisting([CONFIG_PATH, OLD_CONFIG_PATH]);
...
export function loadState(): LauncherState {
  const path = firstExisting([STATE_PATH, OLD_STATE_PATH]);
